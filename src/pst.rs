const MG_MATERIAL: [i32; 6] = [82, 337, 365, 477, 1025, 0]; // P, N, B, R, Q, K
const EG_MATERIAL: [i32; 6] = [94, 281, 297, 512, 936, 0]; // P, N, B, R, Q, K
...
const EG_PESTO_TABLES: [&[i32; 64]; 6] = [
    &EG_PAWN_TABLE,
    &EG_KNIGHT_TABLE,
    &EG_BISHOP_TABLE,
    &EG_ROOK_TABLE,
...
/// Generates the final Piece-Square Tables at compile time.
const fn generate_pst() -> ([[i32; 64]; 13], [[i32; 64]; 13]) {
    let mut mg_pst = [[0; 64]; 13];
    let mut eg_pst = [[0; 64]; 13];
    let mut p_type = 0;
    while p_type < 6 {
        let white_piece_idx = p_type + 1;
        let black_piece_idx = p_type + 7;

        let mut sq = 0;
        while sq < 64 {
            let flipped_sq = sq ^ 56;

            // White pieces
            mg_pst[white_piece_idx][sq] = MG_MATERIAL[p_type] + MG_PESTO_TABLES[p_type][sq];
            eg_pst[white_piece_idx][sq] = EG_MATERIAL[p_type] + EG_PESTO_TABLES[p_type][sq];

            // Black pieces
            mg_pst[black_piece_idx][sq] =
                -(MG_MATERIAL[p_type] + MG_PESTO_TABLES[p_type][flipped_sq]);
            eg_pst[black_piece_idx][sq] =
                -(EG_MATERIAL[p_type] + EG_PESTO_TABLES[p_type][flipped_sq]);
...
const GENERATED_PSTS: ([[i32; 64]; 13], [[i32; 64]; 13]) = generate_pst();

pub const MG_PST: [[i32; 64]; 13] = GENERATED_PSTS.0;
pub const EG_PST: [[i32; 64]; 13] = GENERATED_PSTS.1;
