...
pub fn run_uci() {
    ...
    loop {
            ...
            if let Some((name, value)) = parse_setoption(rest) {
                if name.eq_ignore_ascii_case("Hash") {
                    if let Ok(size) = value.parse::<usize>() {
                        if (1..=4096).contains(&size) && tt_size_mb != size {
                            tt_size_mb = size;
        ...
        if cmd.eq_ignore_ascii_case("ponderhit") {
            // No special handling here; background search continues.
            continue;
        }
        ...
        if let Some(rest) = cmd.strip_prefix("go") {
            ...
            let helper_depth = depth.min(64); // Why: helpers shouldn’t recurse overly deep
            ...
            if is_ponder {
                ...
                if ponder.enabled {
                    ...
                    let handle = builder
                        .spawn(move || {
                            // Spawn helpers with large stacks too.
            ...
            // Helper threads with larger stacks
            let mut helpers = vec![];
            for i in 0..threads_count.saturating_sub(1) {
                let board_clone = b.clone();
